...
contract TokenStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        require(_rewardsToken != address(0), "rewardToken m ust not 0x");
        require(_stakingToken != address(0), "stakingToken must not 0x");
        require(
            _rewardsDistribution != address(0),
            "rewardDistribution must not 0x"
        );
    ...
    function withdraw(
        ...
        require(
            lastStakeTime[msg.sender] + lockPeriod <= block.timestamp,
            "Still locked"
        );
        _totalSupply -= amount;
        _balances[msg.sender] -= amount;
    ...
    function getRewardStable(
        string memory strCoin
    ) public payable nonReentrant updateReward(msg.sender) {
        require(
            lastStakeTime[msg.sender] + lockPeriod <= block.timestamp,
            "Still locked"
        );
